...
export default function CheckoutPage() {
  ...
  useEffect(() => {
    try {
      ...
      if (savedData) {
        ...
        console.log('localStorage에서 결제 정보가 삭제되었습니다.'); // 확인용 로그

      } else {
        
        setCheckoutData(null);
        console.log('localStorage에 결제 정보가 없습니다.'); // 확인용 로그
      }
    } catch (error) {
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto bg-white rounded shadow mt-6">
      ...
      <div className="mb-6"> 
        ...
        <ul className="border rounded p-4 space-y-3">
          ...
          {checkoutData.items.map((item) => (
            <li key={item.productId} className="flex justify-between items-center border-b pb-2 last:border-b-0 last:pb-0">
              <div>
      ...
      <div className="flex justify-center mt-8"> {/* 이 div 안에 버튼을 넣을 거야! */}
        <button
          onClick={() => router.push('/')} // 버튼 클릭 시 router.push('/')로 홈 화면 이동!
          className="px-6 py-2 bg-blue-600 text-white rounded-md shadow hover:bg-blue-700 transition-colors"
        >
          홈으로 가기
        </button>
